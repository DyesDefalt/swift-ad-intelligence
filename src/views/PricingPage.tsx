      ...
      <div className="w-full px-[7vw] relative z-10">
        <div className={`max-w-3xl mx-auto text-center transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--gapah-accent)]/10 border border-[var(--gapah-accent)]/20 mb-8">
            ...
            <span className="text-sm font-medium text-[var(--gapah-accent)]">Pricing</span>
          </div>
          ...
          <p className="text-xl text-[var(--gapah-text-secondary)] max-w-2xl mx-auto leading-relaxed mb-4">
            Choose the plan that fits your needs. All plans include access to all platforms and features.
          </p>
          <p className="text-sm text-[var(--gapah-text-secondary)]">Pricing in Indonesian Rupiah</p>
        </div>
      </div>
    </section>
  );
  ...
  const plans = [
    {
      ...
      features: ['Lifetime license', 'BYOK/BYOAI included', 'All platforms (TikTok, Meta, Google, X)', 'Universal/Advanced mode', 'All features included', 'Priority support', 'Future updates'],
      highlighted: true, cta: 'Get Early Access', badge: 'Best Value', icon: Sparkles,
    },
    {
      name: 'Managed Pro', price: 'Rp 149.000', originalPrice: 'Rp 299.000', period: '/month',
      description: 'For regular users who want managed AI.',
      features: ['500 generations/month', 'All platforms included', 'Universal/Advanced mode', 'Priority support', 'No API key needed', 'Analytics dashboard'],
      highlighted: false, cta: 'Start Pro Trial', badge: null, icon: Zap,
    },
    {
      ...
      highlighted: false, cta: 'Contact Sales', badge: null, icon: Users,
    },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <div key={index} className={`scroll-reveal stagger-${index + 1} ${isVisible ? 'revealed' : ''}`}
              onMouseEnter={() => setHoveredPlan(index)} onMouseLeave={() => setHoveredPlan(null)}>
              <div className={`relative rounded-3xl p-8 h-full transition-all duration-500 ${
                plan.highlighted ? 'bg-white border-2 border-[var(--gapah-accent)] shadow-xl md:scale-105' : 'bg-white/70 backdrop-blur-sm border border-black/5 hover:shadow-lg'
              } ${hoveredPlan === index ? 'scale-[1.02]' : ''}`}>
                {plan.badge && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <span className="bg-gradient-to-r from-[var(--gapah-accent)] to-[#2563EB] text-white text-xs font-bold px-5 py-2 rounded-full shadow-lg">{plan.badge}</span>
                  </div>
                )}
                ...
                <div className="flex items-baseline gap-2 mb-1">
                  ...
                  {plan.originalPrice && <span className="price-original">{plan.originalPrice}</span>}
                </div>
                ...
                <button className={`w-full gapah-button text-sm ${plan.highlighted ? 'gapah-button-primary' : 'gapah-button-secondary'}`}>{plan.cta}</button>
              </div>
            </div>
          ))}
        </div>
        <div className={`mt-12 text-center scroll-reveal stagger-4 ${isVisible ? 'revealed' : ''}`}>
          <p className="text-sm text-[var(--gapah-text-secondary)]"><Shield className="w-4 h-4 inline mr-2" />30-day money-back guarantee. No questions asked.</p>
        </div>
      </div>
    </section>
  );
              ...
              {features.map((feature, index) => (
                <tr key={index} className="border-b border-black/5">
                  ...
                  <td className="text-center py-4 px-4">
                    {typeof feature.early === 'boolean' ? (feature.early ? <Check className="w-5 h-5 text-[var(--gapah-accent)] mx-auto" /> : <span className="text-gray-300">&mdash;</span>) : <span className="text-sm text-[var(--gapah-text-secondary)]">{feature.early}</span>}
                  </td>
                  <td className="text-center py-4 px-4 bg-[var(--gapah-accent)]/5">
                    {typeof feature.pro === 'boolean' ? (feature.pro ? <Check className="w-5 h-5 text-[var(--gapah-accent)] mx-auto" /> : <span className="text-gray-300">&mdash;</span>) : <span className="text-sm text-[var(--gapah-text-secondary)]">{feature.pro}</span>}
                  </td>
                  <td className="text-center py-4 px-4">
                    {typeof feature.agency === 'boolean' ? (feature.agency ? <Check className="w-5 h-5 text-[var(--gapah-accent)] mx-auto" /> : <span className="text-gray-300">&mdash;</span>) : <span className="text-sm text-[var(--gapah-text-secondary)]">{feature.agency}</span>}
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    <section ref={sectionRef} className="py-32 bg-[var(--gapah-dark)] relative overflow-hidden">
      ...
      <div className="w-full px-[7vw] relative z-10">
        <div className={`max-w-3xl mx-auto text-center scroll-reveal ${isVisible ? 'revealed' : ''}`}>
          <h2 className="text-[clamp(40px,5vw,64px)] font-bold text-white mb-6">Still have <span className="text-gradient">questions?</span></h2>
          <p className="text-xl text-white/60 mb-10 leading-relaxed max-w-xl mx-auto">Our team is here to help. Reach out and we will get back to you within 24 hours.</p>
          <div className="flex flex-wrap justify-center gap-4">
            <a href="/contact" className="gapah-button gapah-button-primary text-base py-4 px-8">Contact us<ArrowRight className="w-5 h-5 ml-2" /></a>
          </div>
        </div>
      </div>
    </section>
